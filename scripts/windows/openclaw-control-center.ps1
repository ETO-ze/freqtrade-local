...
$windowsScriptRoot = Join-Path $root 'scripts\windows'
...
$guidePath = Join-Path $root 'docs\guides\OPENCLAW_FREQTRADE_GUIDE.md'
...
do {
    ...
    switch ($choice) {
        '1' {
            powershell -ExecutionPolicy Bypass -File (Join-Path $windowsScriptRoot 'start-openclaw-factor-daemon-fast.ps1')
            Pause-And-Return
        }
        '2' {
            powershell -ExecutionPolicy Bypass -File (Join-Path $windowsScriptRoot 'stop-openclaw-factor-daemon-fast.ps1')
            Pause-And-Return
        }
        '3' {
            powershell -ExecutionPolicy Bypass -File (Join-Path $windowsScriptRoot 'start-openclaw-factor-daemon-stable.ps1')
            Pause-And-Return
        }
        '4' {
            powershell -ExecutionPolicy Bypass -File (Join-Path $windowsScriptRoot 'stop-openclaw-factor-daemon-stable.ps1')
            Pause-And-Return
        }
        '5' {
            powershell -ExecutionPolicy Bypass -File (Join-Path $windowsScriptRoot 'start-openclaw-factor-daemon-evolution.ps1')
            Pause-And-Return
        }
        '6' {
            powershell -ExecutionPolicy Bypass -File (Join-Path $windowsScriptRoot 'stop-openclaw-factor-daemon-evolution.ps1')
            Pause-And-Return
        }
        '7' {
            Start-Process powershell -ArgumentList @('-ExecutionPolicy', 'Bypass', '-File', (Join-Path $windowsScriptRoot 'start-factor-lab.ps1'))
        }
        '8' {
        ...
        '9' {
            powershell -ExecutionPolicy Bypass -File (Join-Path $windowsScriptRoot 'start-openclaw-on-login.ps1')
            Pause-And-Return
        }
        '10' {
        ...
        '17' {
            Start-Process powershell -ArgumentList @('-ExecutionPolicy', 'Bypass', '-Command', "py `"$root\apps\desktop\control_center_gui.py`"")
        }
        '18' {
        ...
        '19' {
            powershell -ExecutionPolicy Bypass -File (Join-Path $windowsScriptRoot 'stop-mainstream-auto-bot.ps1')
            Pause-And-Return
        }
        '20' {
