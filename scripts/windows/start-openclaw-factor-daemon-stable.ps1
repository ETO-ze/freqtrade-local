...
if ($existingDaemon) {
    ...
    if (-not $ForceRestart.IsPresent) {
        Write-Host "OpenClaw stable daemon process appears to be running already. PID: $($existingDaemon.ProcessId), status: $statusName" -ForegroundColor Yellow
        exit 0
    }
    try {
...
if ($existingPid) {
    ...
    if ($pidProcess) {
        $statusName = if ($statusData) { [string]$statusData.status } else { '' }
        if (-not $ForceRestart.IsPresent) {
            Write-Host "OpenClaw stable daemon PID file points to a live process already. PID: $existingPid, status: $statusName" -ForegroundColor Yellow
            exit 0
        }
        try {
