...
$lockPath = Join-Path $daemonReportDir 'factor-daemon-evolution.lock'
...
foreach ($daemon in $matchingDaemons) {
    try {
        ...
        Write-Host "Stopped evolution daemon PID: $($daemon.ProcessId)" -ForegroundColor Cyan
    }
    catch {
        Write-Host "Failed to stop evolution daemon PID $($daemon.ProcessId): $($_.Exception.Message)" -ForegroundColor Yellow
    }
}
